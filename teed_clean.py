...
class DoubleFusion(nn.Module):
    """Fuse two feature maps via element-wise addition + DoubleConv."""
...
class UpBlock(nn.Module):
    """Upsample (bilinear) + DoubleConv + DoubleFusion."""

    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv = DoubleConv(in_ch, out_ch, in_ch // 2)
        self.fusion = DoubleFusion(out_ch, out_ch)
...
class SideHead(nn.Module):
    """1x1 conv -> upsample -> sigmoid side output."""

    def __init__(self, in_ch, out_ch=1):
        super().__init__()
        self.side = nn.Conv2d(in_ch, out_ch, kernel_size=1)
        self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
...
class TED(nn.Module):
    """TED: Tiny and Efficient Edge Detector."""
    ...
    def __init__(self, in_channels=3, base_channels=16):
        ...
        # ── Decoder ──
        self.up1 = UpBlock(base_channels * 12, base_channels * 4)
        self.up2 = UpBlock(base_channels * 6, base_channels * 2)
        self.up3 = UpBlock(base_channels * 3, base_channels)

        # ── Side outputs ──
        self.side1 = SideHead(base_channels * 8)
        self.side2 = SideHead(base_channels * 4)
        self.side3 = SideHead(base_channels * 2)
        self.side4 = SideHead(base_channels)

        # ── Double Fusion blocks ──
        self.af1 = DoubleFusion(base_channels * 4, base_channels * 4)
        self.af2 = DoubleFusion(base_channels * 2, base_channels * 2)
        self.af3 = DoubleFusion(base_channels, base_channels)

    def forward(self, x, single_test=False):
        if single_test:
            x = self._pad_to_multiple(x, multiple=8)
        ...
        d2 = self.up2(d1_af, x2)
        d2_af = self.af2(d2)

        d3 = self.up3(d2_af, x1)
        d3_af = self.af3(d3)

        # ── Side outputs ──
        o1 = self.side1(x4, original_size)
        o2 = self.side2(d1, original_size)
        o3 = self.side3(d2, original_size)
        o4 = self.side4(d3_af, original_size)
